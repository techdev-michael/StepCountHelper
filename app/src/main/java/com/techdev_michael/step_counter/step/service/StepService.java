package com.techdev_michael.step_counter.step.service;
...
import com.techdev_michael.step_counter.activity.MainActivity;
import com.techdev_michael.step_counter.step.UpdateUiCallBack;
import com.techdev_michael.step_counter.step.accelerometer.StepCount;
import com.techdev_michael.step_counter.step.accelerometer.StepValuePassListener;
import com.techdev_michael.step_counter.step.bean.State;
import com.techdev_michael.step_counter.step.bean.StepData;
import com.techdev_michael.step_counter.step.utils.DbUtils;
...
public class StepService extends Service implements SensorEventListener {
     ...
     * 当前所走的步数
     */
    private int CURRENT_STEP, CURRENT_RUN_STEP;
    /**
    ...
    private void initTodayData() {
        ...
        if (list.isEmpty()) {
            ...
            CURRENT_RUN_STEP = Integer.parseInt(list.get(0).getRunSteps());
        } else {
            Log.v(TAG, "出错了！");
        }
        if (mStepCount != null) {
            mStepCount.setSteps(CURRENT_STEP, CURRENT_RUN_STEP);
        }
        updateNotification();
    }
...
//                    Logger.d("重置步数" + StepDcretor.CURRENT_STEP);
    ...
    private void updateNotification() {
        ...
        if (mCallback != null) {
            mCallback.updateUi(CURRENT_STEP, CURRENT_RUN_STEP);
        }
    ...
    public int[] getStepCount() {
        return new int[]{CURRENT_STEP, CURRENT_RUN_STEP};
    }
     ...
     * <p>
     * 2.TYPE_STEP_DETECTOR 翻译过来就是走路检测，
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        Log.e(TAG, "onSensorChanged: 传感器接受到变化信息");
    ...
    private void addBasePedometerListener() {
        ...
        mStepCount.initListener(new StepValuePassListener() {
            ...
            @Override
            public void stepChanged(int steps, int runSteps) {
                CURRENT_STEP = steps;
                CURRENT_RUN_STEP = runSteps;
                updateNotification();
            }
        ...
        boolean isGyroscopeAvailable = sensorManager.registerListener(mStepCount.getStepDetector(), gyroscopeSensor,
                SensorManager.SENSOR_DELAY_NORMAL);

        if (isGyroscopeAvailable) {
            Log.e(TAG, "陀螺仪传感器可以使用");
        } else {
            Log.e(TAG, "陀螺仪传感器不可以使用");
        }
    ...
    class TimeCount extends CountDownTimer {
        ...
        public void onTick(long millisUntilFinished) {
            ...
            if (CURRENT_STEP == lastStep) {
                if (null != mCallback) {
                    mCallback.updateState(State.STOP);
                }
            }
    ...
    private void save() {
        ...
        if (list.isEmpty()) {
            ...
            data.setRunSteps(String.valueOf(CURRENT_RUN_STEP));
            DbUtils.insert(data);
        } else if (list.size() == 1) {
